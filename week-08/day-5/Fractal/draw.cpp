...
void draw::draw_equal_triangle_down(SDL_Renderer* &renderer, int x, int y, int line) {
  setX(x);
  setY(y);
  DrawTo(renderer, x + line, y);
  DrawTo(renderer, x + line / 2, y + sqrt(line * line - line/2 * line/2));
  DrawTo(renderer, x, y);
}

void draw::draw_equal_triangle_up(SDL_Renderer* &renderer, int x, int y, int line) {
  setX(x);
  setY(y);
  DrawTo(renderer, x + line, y);
  DrawTo(renderer, x + line / 2, y - sqrt(line * line - line / 2 * line / 2));
...
void draw::sierpinsky_triangle(SDL_Renderer* &renderer, int x, int y, int line) {
  ...
  if (line > 20) {
   // fill_sierpinsky_triangle(renderer, x, y, line);
    ...
    sierpinsky_triangle(renderer, x + line / 4, y + sqrt(line * line - line / 2 * line / 2) / 2, line / 2);
  }
}
